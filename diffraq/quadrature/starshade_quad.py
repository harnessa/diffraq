...
def starshade_quad(num_pet, Afunc, r0, r1, m, n):
    ...
    Inputs:
        ...
        r0, r1 = apodization domain of radii. r<r0: A=1; r>r1: A=0
    ...
    xq, yq, wq = diffraq.quad.polar_quad(lambda t: r0*np.ones_like(t), m, nd)

    ### Build over petals ###

    #Petals width nodes and weights over [-1,1]
    pw, ww = diffraq.quad.lgwt(n, -1, 1)
    ...
    #thetas
    tt = np.kron(np.ones(num_pet), (np.pi/num_pet) * Aval[:,None] * pw) + \
         np.kron(2.*np.pi*(np.arange(num_pet) + 1), np.ones(n))

    #Add Petal nodes + weights
    xq = np.concatenate((xq, (pr[:,None] * np.cos(tt)).flatten() ))
    yq = np.concatenate((yq, (pr[:,None] * np.sin(tt)).flatten() ))
    wq = np.concatenate((wq, (np.pi/num_pet) * ((wi*Aval)[:,None] * ww).flatten() ))
